...
float4 main(PSInput input) : SV_TARGET
{
	...
	float3 diffuse = max(0.f, dot(light_dir, normal_dir)) * g_light_color_.xyz * g_base_color_.xyz;
	...
	float3 specular = pow(max(0.f, dot(reflect_dir, view_dir)), max(8.f, g_gloss_)) * g_specular_color_.xyz;
