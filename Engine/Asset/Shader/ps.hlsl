...
float3 CalculateLight(Light light, PSInput input)
{
		...
		linear_color = falloff * light.light_intensity_ * (light.light_color_ * g_base_color_.rgb * max(dot(N, LDir), 0.f) +
			light.light_color_ * pow(max(0.f, dot(R, V)), max(4.f, g_gloss_)));
	}
	return linear_color;

}

float shadow_test(PSInput input,Light light)
{
	int shadow_map_index = light.shadow_map_index;
	if (shadow_map_index == -1 || shadow_map_index > 7)
		return 1.f;
	float closest_depth;
	if (light.light_type_ == 1)
	{
		closest_depth = g_cube_shadow_map[0].Sample(g_sampler, normalize(input.positionW - light.light_pos_)).x * 10000.f;
	}
	else
	{
	...
	float bias = 0.5f;
	float cur_depth = length(input.positionW - light.light_pos_);
	...
	SV_TARGET
{
		...
		for (int i = 0; i < 40; ++i)
		{
			float visbility = shadow_test_point_light(lights[i].shadow_map_index,input.positionW,lights[i].light_pos_);
